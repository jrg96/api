...
class Pagomodificar extends CI_Controller {
    ...
    public function index($id_datos_cliente, $id_caso_juridico, $id_pago_caso)
    {
        ...
        $this->smarty->view('pago_modificar.php');
    }
	...
	public function procesar()
	{
		...
		////////////////////////// Validacion de datos ///////////////////////////
		if ( empty($id_pago_caso) || empty($id_datos_cliente) || empty($id_caso_juridico) || empty($monto_pago)
		...
		if ($valido)
		{
			...
			$resultado = $this->PagoCaso_model->actualizar_datos_pago_caso($id_pago_caso, $id_caso_juridico, $monto_pago, $fecha_pago_f, $descripcion, $estado_pago);
			
			// Registrar accion
			$this->UsuarioSistema_model->registrar_accion($this->session->userdata('id_usuario'), "Usuario ha modificado pago ID=$id_pago_caso");
		...
		redirect('/pagomodificar/index/' . $id_datos_cliente . '/' . $id_caso_juridico . '/' . $id_pago_caso);
	}
}
