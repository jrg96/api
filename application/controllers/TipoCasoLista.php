...
class Tipocasolista extends CI_Controller {
    ...
    public function index()
    {
		...
		// Registrar accion
		$this->UsuarioSistema_model->registrar_accion($this->session->userdata('id_usuario'), "Usuario ha accedido a la pantalla de lista de tipos de caso del sistema de información gerencial");
		...
		$total_records = $this->UsuarioSistema_model->obtener_total_usuarios();
		...
		if ($total_records > 0){
			$params['resultados'] = $this->TipoCaso_model->obtener_lista_tipo_caso_juridico($limite_por_pagina, $inicio_indice);
			
			// parametros para realizar la paginacion
			$config['base_url'] = base_url() . 'index.php/tipocasolista/index';
        ...
        $this->smarty->view('tipocaso_lista.php');
    }
}
